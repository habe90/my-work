...
class InvoiceController extends Controller
{
    ...
    public function generateInvoices()
    {
        try {
            ...
            foreach ($successfulJobs as $company_id => $jobs) {
                ...
                if (empty($jobs) || is_null($company_id)) {
                    continue;
                }
    ...
    {
        $invoice = Invoice::with('company')->where('id', $invoiceId)->firstOrFail();
    ...
    {
        $invoice = Invoice::with(['company', 'successfulJobs.offer'])->findOrFail($invoiceId);
